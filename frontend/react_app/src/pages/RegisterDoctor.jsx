...
import { adminAPI, hospitalAPI } from '../services/api.js';
...
function RegisterDoctor({ setAdminLoggedIn }) {
  ...
  useEffect(() => {
    async function fetchHospitals() {
      try {
        console.log('Fetching hospitals...', hospitalAPI);
        if (typeof hospitalAPI !== 'undefined' && hospitalAPI.getHospitals) {
          const response = await hospitalAPI.getHospitals();
          console.log('Hospitals response:', response);
          if (response && response.success && response.data) {
            // Handle both paginated and non-paginated responses
            const hospitalList = Array.isArray(response.data)
              ? response.data
              : (response.data.results || []);
            setHospitals(hospitalList);
            console.log('Hospitals set:', hospitalList);
          }
        }
      } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.firstName || !form.lastName || !form.email || !form.phone || !form.specialty || !form.department || !form.license || !form.qualification) {
      setError('Please fill all required fields (Name, Email, Phone, Specialization, License, and Qualification).');
      setLoading(false);
      return;
    }
  ...
  return (
    <AdminLayout active="register-doctor" setAdminLoggedIn={setAdminLoggedIn}>
      ...
      <form className="register-doctor-form" onSubmit={handleSubmit}>
        <section className="form-section branch-info">
          <h3>Hospital Branch Selection</h3>
